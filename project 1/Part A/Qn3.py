...
if __name__ == '__main__':
    p = Pool(5)
    paras = p.map(main, num_neurons)
    paras = np.array(paras)
    train_acc, test_acc, mean_acc, test_mean_acc = paras[:, 0], paras[:, 1], paras[:, 2], paras[:, 3]
    p.close()
    p.join()

    # for no_neurons in range(len(num_neurons)):
    #     print('validating with neurons size of:', num_neurons[no_neurons])
    #     train_acc, test_acc, mean_acc = main(num_neurons[no_neurons])
    ...
    for i in range(5):
        ...
        plt.legend(
            ["5 hidden neurons", "10 hidden neurons", "15 hidden neurons", "20 hidden neurons", "25 hidden neurons"],
            loc='lower right')
        plt.title('Training Accuracy')
        plt.xlabel(str(epochs) + ' Epochs')
        plt.ylabel('Cross Validation Accuracy')
        plt.savefig('./figures/PartA_Qn3a_trainAcc.png')
        ...
        plt.xlabel(str(epochs) + ' Epochs')
        plt.ylabel('Cross Validation Accuracy')
        plt.savefig('./figures/PartA_Qn3a_testAcc.png')

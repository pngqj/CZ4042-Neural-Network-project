...
if __name__ == '__main__':
    ...
    # Multiprocessing
    p = Pool(5)
    paras = p.map(main, beta)
    paras = np.array(paras)
    train_acc, test_acc, mean_acc, test_mean_acc = paras[:, 0], paras[:, 1], paras[:, 2], paras[:, 3]
    p.close()
    p.join()

    # for no_beta in range(len(beta)):
    #     print('validating with decay beta weight of:', beta[no_beta])
    #     train_acc, test_acc, mean_acc = main(beta[no_beta])
    ...
    for i in range(5):
        ...
        plt.legend(
            ["Decay Beta 0", "Decay Beta 1e-3", "Decay Beta 1e-6", "Decay Beta 1e-9", "Decay Beta 1e-12"],
            loc='lower right')
        plt.title('Training Accuracy')
        plt.xlabel(str(epochs) + ' Epochs')
        plt.ylabel('Cross Validation Accuracy')
        plt.savefig('./figures/PartA_Qn4a_trainAcc.png')
        ...
        plt.xlabel(str(epochs) + ' Epochs')
        plt.ylabel('Cross Validation Accuracy')
        plt.savefig('./figures/PartA_Qn4a_testAcc.png')

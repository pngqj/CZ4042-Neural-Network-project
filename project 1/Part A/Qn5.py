...
def fnn(x, hidden_units, hidden_units2):
    # Hidden 1
    ...
    h2_weights = tf.Variable(
        tf.random.truncated_normal([hidden_units, hidden_units2], stddev=1.0 / math.sqrt(float(NUM_FEATURES))),
        name='weights')
    h2_biases = tf.Variable(tf.zeros([hidden_units2]), name='biases')

    h2 = tf.nn.relu(tf.matmul(h, h2_weights) + h2_biases)

    # Output layer
    weights = tf.Variable(
        tf.random.truncated_normal([hidden_units2, NUM_CLASSES], stddev=1.0 / math.sqrt(float(NUM_FEATURES))),
        name='weights')
    biases = tf.Variable(tf.zeros([NUM_CLASSES]), name='biases')
    logits = tf.matmul(h2, weights) + biases

    return logits, h_weights, h2_weights, weights
...
def main():
    ...
    logits, h_weights, h2_weights, weights = fnn(x, num_neurons, num_neurons)
    ...
    plt.savefig('./figures/PartA_Qn5_Acc.png')
    ...
    plt.savefig('./figures/PartA_Qn5_Err.png')
